...
#define PT_DOCUMENT    CApp::pCApp()->getDocumentPtr()
...
Paramètre N° 3 (facultatif : format du type de date à retourner) Le format des dates. si non précise dd-MM-yyyy (voir : Format des dates)  .
*/
/*! \brief non documente */
...
{if (u_Html() == 0)        return TR("::LOAD_URL_IN_VAR() m_pC_Utils_Html est null et non instanci\303\251 ");
 ...
 //CGestIni::Param_UpdateToDisk("/home/ro/LOAD_URL_IN_VAR_URL.html", url);
 //CGestIni::Param_UpdateToDisk("/home/ro/LOAD_URL_IN_VAR.html", (*PT_VAR_MAP)[var_name]);
 return QString::null;
}
...
{if (pQWebView() == 0)        return TR("::GET_DISPLAY_PAGE() m_pQWebView est null et non instanci\303\251 ");
 ...
 //CGestIni::Param_UpdateToDisk("/home/ro/GET_DISPLAY_PAGE.html", html);
 return QString::null;
}
...
{if (patern_deb.length()==0 || patern_end.length()==0) return QString::null;
 QString   resolvToken    = "";
 long     end_patern_deb  = 0;
 long     deb_Patern_end  = 0;
