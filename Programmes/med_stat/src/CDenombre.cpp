...
//----------------------------------------------- DoResult ----------------------------------------------------------
void CDenombre::DoResult(QSqlQuery &query)
{QString res;
 if (m_FieldWork[0]=='*')
    {if (m_pCMetaField)
        {int   nb      = m_pCMetaField->m_ResultList.count();
         for (int i=0; i<nb; ++i)
             { res = m_pCMetaField->m_ResultList[i];
               res = res.remove("\t");
               res = res.remove("\r");
               res = res.remove("\n");
    ...
    {res = CGestIni::Utf8_Query(query, m_Field_ID );
     AddResult(res.stripWhiteSpace());
    }
}
