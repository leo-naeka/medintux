 ...
 * \sa exeFunction()
 */
int C_TokenInterpret::ResolvMacroToken(CDevilCrucible *pCDC, QString &resolvToken, QString token, QString &)
...
QString C_TokenInterpret::NOUVEAU_DOCUMENT(QStringList &arg_list)
    {if (G_mCDC->m_isJustForWatch==CDevilCrucible::JustForWatch) return QString("");
     ...
     if (pCMDI_Generic!=0) pCMDI_Generic->AddNewDocument(" ",  pCMDI_Generic->GetType() );
     return "";
    }
//-------------------------- OS -------------------------------------------
/*! \brief non documenté */
...
QString C_TokenInterpret::OPEN_FILE_NAME(QStringList &arg_list)
    {QString preselect      = "";
     ...
     if (arg_list.count()>=2)  dirPresel = G_pCApp->resolvePath(arg_list[0].stripWhiteSpace());
...
{char *deb = (char*)(const char*) str;
 ...
 while(pt && *pt)
    {if (pt[0] == '\\') pt += 2;
        ...
        {if (pt[0] == delimiter)      return pt - deb;
         ++pt;
        }
    }
                                      return -1;
