 ...
 * Avant d'insérer l'item envoyé, teste si il existe un fichier .dir.html dans le repértoire qui remplacera le contenu à insérer. Autrement dit, soit il existe un fichier $GLOSSAIRE/item/FILE_NAME_REPLACOR alors c'est le contenu du fichier qui est inséré soit ce fichier n'existe pas et c'est l'item qui est inséré dans le texte. \n
...
void MyEditText::PasteFolderItem ( )
{
 ...
 if (Item_Compose == -1)        // test si m_ListItem est un item simple (pour être certain !!!)
  {  // teste l'existence du fichier FILE_NAME_REPLACOR
     QString file_replacor = "";
     QString path = "";
     path               = G_pCApp->m_PathGlossaire  + INSERTION_ROOT + QDir::separator() + m_ListItem;
     file_replacor      = path + QDir::separator() + FILE_NAME_REPLACOR;
     if (QFile::exists ( file_replacor ) )  // reste à tester la taille du fichier
      { PasteFileItem (file_replacor); }
     else
        { QDir dir ( path );
          // test l'existence de la liste sur le disque dur (nécessaire à cause du fichier cache...
          if (dir.exists())
            {   setCurrentFont(m_LastFont);
                setUnderline(FALSE);
                if (m_LastColor == QColor(0x00,0x00,0xFF)) setColor(QColor(0x00,0x00,0x00));
                                        ...
                                        int nbSubMenu /*=0*/)
{
  ...
  while ( (fi = it.current()) != 0 )
    {QString fname = fi->fileName();//.latin1();
     if (fname == ".." || fname == "." || fname == FILE_NAME_REPLACOR || fname == DIR_CACHE_LISTES)
              {
              }
     else if (fi->isDir() && fname != "." && fname.lower() != "cvs")            // *** c'est une autre liste ***
...
{ //int i=0;
  ...
  while ( (fi = it.current()) != 0 )
    { if ((fi->fileName() != ".") && fi->fileName() != "..")
       {
        QString fileCache = fi->fileName();
        QStringList *liste = new QStringList;
        if (!QFile::exists ( pathCache + QDir::separator() + fileCache ) )
         { // le fichier n'existe pas ==> le construire pour l'utiliser
          QString thisListName = fi->fileName();
          GetListForCache(*liste, thisListName ,"",0);

          QFile file( pathCache + QDir::separator() + fileCache );
...
{ int i = 0;
  ...
  // Liste qui contiendra le cache du menu
  QStringList *liste = new QStringList;
