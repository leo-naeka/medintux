...
{   m_PathAppli                    = pathAppli;
    ...
    m_MonthLabelCss = CGestIni::Param_ReadUniqueParam(m_LocalParam.toAscii(),                                                "Agenda", "Style du label mois").replace("_",",");
    ...
    m_pCMoteurAgenda->SetAgendaMode_WeekOrDayOrMonth(CGestIni::Param_ReadUniqueParam(m_LocalParam.toAscii(),                 "Agenda", "Affichage Jour ou Semaine"));
...
void C_Frm_Agenda::toGoogle(const QDateTime &dateDeb, const QDateTime &dateEnd)
{
 ...
 if (m_pC_GoogleAPI->getError().length())QMessageBox::question ((QWidget*)parent(),
                                                                ...
                                                                tr("<b>Error C_Frm_Agenda::toGoogle() : error in Google transaction:</b><br>%1").arg(m_pC_GoogleAPI->getError()),
                                                                QMessageBox::Cancel);
 QApplication::restoreOverrideCursor();
}
//---------------------------------------- GoogleConnectionErrorDisplay --------------------------------------------
...
{emit Sign_ReinitModeWeekDayMonth_OnDate(m_tmpDateToStart, "WEEK", this);
}
...
void C_Frm_Day::replaceRdvByRdv(C_Frm_Rdv *pC_Frm_Rdv_Dst, const C_RendezVous &rdvSrc)
{
  ...
  pC_Frm_Rdv_Dst->RDV_Update(); // mettre à jour le RDV la liste du cache et enregistrer dans la base
}
