...
public:
    ...
    C_BDM_DrugListRecord(QString id,
     ...
     QString ucd()   const         {return m_ucd;}
     ...
     void set_id(QString value)             {m_id             = value;}
     void set_id_type(QString value)        {m_id_type        = value;}
     void set_commercialName(QString value) {m_commercialName = value;}
     void set_dci_1Name(QString value)      {m_dci_1Name      = value;}
     void set_dci_2Name(QString value)      {m_dci_2Name      = value;}
     void set_dci_3Name(QString value)      {m_dci_3Name      = value;}
     void set_owner(QString value)          {m_owner          = value;}
     void set_lang(QString value)           {m_lang           = value;}
     void set_ucd(QString value)            {m_ucd            = value;}
     void set_ucd_price(QString value)      {m_ucd_price      = value;}
     void set_atc(QString value)            {m_atc            = value;}
     void set_generique(QString value)      {m_generique      = value;}
     void set_statut(QString value)         {m_statut         = value;}

     QString                       serialize(int with_Header = 1 , const QString &ofset  ="" ) const;
     static QString                serialize(const C_BDM_DrugListRecord *pC_BDM_DrugListRecord, int with_Header = 1 , const QString &ofset ="" );
...
public:
    ...
    C_OrdoLineRecord( const C_BDM_DrugListRecord &bdm_DrugListRecord)
        : C_BDM_DrugListRecord( bdm_DrugListRecord.id(),
                                ...
                                bdm_DrugListRecord.ucd(),
                                bdm_DrugListRecord.ucd_price(),
                                bdm_DrugListRecord.atc(),
                                bdm_DrugListRecord.generique(),
                                bdm_DrugListRecord.statut())
    {   /*
        m_id             = ordoLineRecord.id();
        m_id_type        = ordoLineRecord.id_type();
        m_commercialName = ordoLineRecord.commercialName();
        m_dci_1Name      = ordoLineRecord.dci_1Name();
        m_dci_2Name      = ordoLineRecord.dci_2Name();
        m_dci_3Name      = ordoLineRecord.dci_3Name();
        m_owner          = ordoLineRecord.owner();
        m_lang           = ordoLineRecord.lang();
        m_ucd            = ordoLineRecord.ucd();
        m_ucd_price      = ordoLineRecord.ucd_price();
        m_atc            = ordoLineRecord.atc();
        m_generique      = ordoLineRecord.generique();
        m_statut         = ordoLineRecord.statut();
        */
        m_ordo_key               =   "";
        m_numOrdre               =   "";
        m_secureState            =   "";
 ...
 private:
     QString   m_ordo_key;        // cette valeur est la clef (exemple: QDateTime de redaction) d'appartenance à une ordonnance composée de plusieurs lignes de C_OrdoLineRecord
     QString   m_numOrdre;        // eventuel numero d'ordre du produit dans l'ordonnance à laquelle il appartient
     ...
     QString   m_smr;                      // SMR lié à cette prescription
...
public:
    ...
    enum indic_flag  { ASMR     = 1,       // we want indications with ASMR level (if exists)
                       ...
                       ALL_TYPE = 12,      // we want all coded indications
                       ALL_INDIC= 15
                     };
    ...
    */
     ...
     */
    virtual QList<C_InterResultPair> getInteractionRedundancy( const QStringList &cipList)=0;
    ...
    */
    int           isInitialised();
    //-------------------------------- patientContexte ------------------------------------------------------
    /*! \brief return a pointer on C_PatientCtx (patient contexte) .
    */
...
//$    void setPatientContexte(C_PatientCtx* patientCtx);
     ...
     *  "345687/123!ALL_LITERAL=comprimé[1;m|2;M|4;s]/8\n[0.5;m|1;M15r|2;sr30|3;h10-13]/4\n[0.5;m|1;s]/3".
 ...
 *  this list is to update each news datas base source versions datas. We proceed update if
 *  m_DatasSourceVersion differs from pC_BDM_PluginI->dataSourceVersion();
 *  so wee needs SQL functions to manage this druglist.
 */
...
public:
    ...
    */
    C_BDM_Api           ( const QString   &confData,                        // definition des champs de la base de donnée (ceux de la liste des drogues)
    ...
    */
     ...
     */
    int selectProductsList (  QTreeWidget *pQTreeWidget,                                      // QTreeWidget graphique à renseigner avec les données
     ...
     * m_DatasSourceVersion differs from pC_BDM_PluginI->dataSourceVersion();
     *  \param QString &id code of the drug from wich wee want indications exemple cip code of the drug.
