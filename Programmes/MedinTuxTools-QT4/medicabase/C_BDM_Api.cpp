...
//---------------------------- serialize ------------------------------------------------
QString C_BDM_DrugListRecord::serialize(const C_BDM_DrugListRecord *pC_BDM_DrugListRecord, int with_Header /* = 1 */, const QString &ofset /* ="" */)
{QString modeleXML = "";
 if (with_Header)
 ...
 modeleXML +=      ofset + "   <version>1</version>\n";
      //............................. C_BDM_DrugListRecord ..............................
      CGestIni::addXmlData("m_id",              QByteArray().append(pC_BDM_DrugListRecord->m_id),                         modeleXML,false, ofset); //pC_RendezVous->m_date.toString(Qt::ISODate), modeleXML,0);
      CGestIni::addXmlData("m_id_type",         QByteArray().append(pC_BDM_DrugListRecord->m_id_type),                    modeleXML,false, ofset);
      CGestIni::addXmlData("m_commercialName",  QByteArray().append(pC_BDM_DrugListRecord->m_commercialName),             modeleXML,false, ofset);
      CGestIni::addXmlData("m_dci_1Name",       QByteArray().append(pC_BDM_DrugListRecord->m_dci_1Name),                  modeleXML,false, ofset);
      CGestIni::addXmlData("m_dci_2Name",       QByteArray().append(pC_BDM_DrugListRecord->m_dci_2Name),                  modeleXML,false, ofset);
      CGestIni::addXmlData("m_dci_3Name",       QByteArray().append(pC_BDM_DrugListRecord->m_dci_3Name),                  modeleXML,false, ofset);
      CGestIni::addXmlData("m_owner",           QByteArray().append(pC_BDM_DrugListRecord->m_owner),                      modeleXML,false, ofset);
      CGestIni::addXmlData("m_lang",            QByteArray().append(pC_BDM_DrugListRecord->m_lang),                       modeleXML,false, ofset);
      CGestIni::addXmlData("m_ucd",             QByteArray().append(pC_BDM_DrugListRecord->m_ucd),                        modeleXML,false, ofset);
      CGestIni::addXmlData("m_ucd_price",       QByteArray().append(pC_BDM_DrugListRecord->m_ucd_price),                  modeleXML,false, ofset);
      CGestIni::addXmlData("m_atc",             QByteArray().append(pC_BDM_DrugListRecord->m_atc),                        modeleXML,false, ofset);
      CGestIni::addXmlData("m_generique",       QByteArray().append(pC_BDM_DrugListRecord->m_generique),                  modeleXML,false, ofset);
      CGestIni::addXmlData("m_statut",          QByteArray().append(pC_BDM_DrugListRecord->m_statut),                     modeleXML,false, ofset);
 modeleXML += ofset +  "</C_BDM_DrugListRecord>\n";
 return modeleXML;
}
...
{   int nextPos=0;
    ...
    obj->m_ucd             = CGestIni::getXmlData("m_ucd",                data, &nextPos);
...
{QString modeleXML =
 ...
 "   <version>1</version>\n";
      ...
      CGestIni::addXmlData("m_ucd",             QByteArray().append(pC_OrdoLineRecord->m_ucd),                        modeleXML,0);
      ...
      CGestIni::addXmlData("m_schema_posologique",   QByteArray().append(pC_OrdoLineRecord->m_schema_posologique),               modeleXML,0);
 modeleXML +=
 "</C_OrdoLineRecord>\n";
 return modeleXML;
}
...
*/
//$void C_BDM_PluginI::setPatientContexte(C_PatientCtx* patientCtx)
//${m_pC_PatientCtx = patientCtx;
//$}

//-------------------------------- patientContexte ------------------------------------------------------
/*! \brief return  patient context.
*/
//$C_PatientCtx* C_BDM_PluginI::patientContexte()
...
/*
                ...
                " <gph_df></gph_df>\n"             //        date et heure du fin  de prise de la prescription format dd-MM-yyyy hh:mm:ss   // la date de fin est calculee avec <gph_cy>
...
QString C_BDM_PluginI::dbi_increasePosologieHit(const QString &grammar, const QString &drug_id, const QString &drug_id_type /* = "CIP" */,  int mode  /* = INCREASE_HIT */)
{
    ...
    //if (sequences.length()==0 && drug_id_type=="CIP")    // si CIP on exige au moins une sequence posologique
    //   {dbi_database()->outMessage( QObject::tr("WARNING : C_BDM_Api::increasePosologieHit() can't doing posologie hit increase because sequences are empty"), __FILE__, __LINE__);  return 0;
    //   }
    nextPos                 = 0;
    ...
    QString ToStore   = QString ("<gph>\n"
                      ...
                      + QString (" <gph_pmin>%1</gph_pmin>\n"         //        posologie minimum
                                ...
                                ).arg(qPA_min_By_FactCorp, qPA_max_By_FactCorp, unitePoso, facteurCorpo, unite_facteurCorpo, qPA_ByUnitPrise)
                      + QString (
                                 " <gph_nm>%1</gph_nm>\n"
                                 " <gph_cm>%2</gph_cm>\n"
                                 " <gph_in>%3</gph_in>\n"
                                 ...
                                 "%5"
                                 "</gps>\n").arg(literalite_N, literalite_T, indicStr, comment, sequences);
    ...
    if (indicParts.count()>=3)
       {
        query.bindValue(5, indicParts[0]); //  m_BDM_POSO_HIT_CODE_INDIC
        query.bindValue(6, indicParts[1]); //  m_BDM_POSO_HIT_CODE_INDIC_TYPE
        query.bindValue(7, indicParts[2]); //  m_BDM_POSO_HIT_LIBELLE_INDIC
       }
    else if( indicParts.count()>=1 )
       {
        query.bindValue(5, "");
        query.bindValue(6, "");
        query.bindValue(7, indicParts[0]);
       }
    else
       {
        query.bindValue(5, "");
...
{QList < C_BDM_DrugListRecord > productsList;
 ...
 QString  requete = " SELECT DISTINCT `" + m_BDM_POSO_HIT_DRUG_ID            + "`,"     // 0    id
                    ...
                    " FROM  "            + m_BDM_POSO_HIT_TBL_NAME           +
                    " GROUP BY `"        + m_BDM_POSO_HIT_DRUG_ID            + "`";
                    " ORDER BY `"        + m_BDM_POSO_HIT_SCORE              + "` DESC LIMIT "        + QString::number(limit);
 productsList.clear();
 QSqlQuery query (requete , dbi_database()->database() );
...
{   if (! dbi_database() )                         return false;
    ...
    QProgressDialog progress(QObject::tr("Initialiasing Drugs List..."), QObject::tr("Abort"), 0, nbRecordsToParse/10, 0);
    ...
    while (getDrugList_NextRecord(c_bdm_druglistrecord))
        {queryInsert.prepare(prepare);
         ...
         queryInsert.bindValue(9,  c_bdm_druglistrecord.ucd());
...
int C_BDM_PluginI::dbi_createPosologieHit(int mode  /* = ONLY_IF_NOT_EXISTS */)
{
    ...
    if (dbi_database()->createTable(
                    ...
                    "`"+m_BDM_POSO_HIT_GRAMMAR         +"` VARCHAR(1024),"                     // grammaire de la posologie liee a ce favoris
...
{m_pC_BDM_PluginI                = 0;
 ...
 if ( ! isThisTableExist(m_BDM_USER_MONOGRAPHIE_TBL_NAME)  &&
      ! createCustomMonographieTable() )                          {m_isValid = 0; return;}
 m_isValid = 1;
}
 ...
 */
 ...
 {   QString optional_icone = "";
     ...
     char    typ            = interResultPair.Typ().toAscii()[0];
     switch (typ)
            { case 'I': { main_icone      = terrainLevel_ToPixmapPath(level);
                          char    typ_c   = interResultPair.Typ().mid(3,1).toAscii()[0];                           // on se place apres le "IT_"
                          switch(typ_c)
              ...
              case 'R': { main_icone      = m_imagePath+"redondances.png";  // REDONDANCES
                          optional_icone  = "";
 ...
 {    QString html_out =  QString ("<table cellSpacing=\"0\"  cellpadding=\"4\" width=100% border=\"1\">"    // #FF8000
                                 "<tbody>"
                                 "<tr>"
                                 "<td width=300% align=\"left\" bgcolor=\"#FEFFDD\">")+
                                 text_in +
                                 "</td>"
                                 "</tbody>"
                                 "</table>";
 ...
 QString C_BDM_Api::evaluateAllInteractions(const QStringList &grammarList)
 {
     ...
     interResultPairList                        += getInteractionAllergiesFromCipList( cipList );
     interResultPairList                        += getInteractionPairFromCipList ( cipList );
     ...
     interResultPairList                        += getInteractionRedundancy( cipList );
 ...
 /*! \brief return QString level interaction between a products ant patient sane state.
 ...
 /*! \brief return QString level interaction between two products.
  *  \param int level of interaction .
  *  \return QString
  */
 ...
 */
  ...
  QString C_BDM_Api::afssaps_LevelToPixmapPath(int level) const
 {
   ...
   *  \return QList<C_InterResultPair> result interaction pair list
   */
  QList<C_InterResultPair> C_BDM_Api::getInteractionRedundancy( const QStringList &cipList)
  {   QList<C_InterResultPair>        interResultPairList;
      if (m_pC_BDM_PluginI) return m_pC_BDM_PluginI->getInteractionRedundancy( cipList );
      return interResultPairList;
  }
...
{   QList<C_InterResultPair>        interResultPairList;
    ...
    for (int i=0; i < cipList.size(); ++i)
        {  cip = cipList[i];
           interResultPairList += getInteractionCim10(cip, m_pC_PatientCtx->cim10List() , CIP_to_DrugName(cip)  );
        }
    return interResultPairList;
}
