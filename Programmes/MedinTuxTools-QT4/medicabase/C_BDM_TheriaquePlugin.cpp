...
QList<C_InterResultPair>  C_BDM_TheriaquePlugin::getInteractionDoseMinMax_fromCIP(  C_PatientCtx *pC_PatientCtx, const QString &grammar)
{
     ...
     /*int up_frequency = */ C_PosologieGrammar::toMinMax_UP(seqList,  q_min,  q_max, up_formes_min_max_period);   // normalement si un factuer corporel existe les valeurs q_min et q_max sont deja multiplees par lui
    ...
    if (isResponse)
       { if (q_min<drug_min)
        ...
        if (q_max>drug_max)
           { interResultPairList.append( C_InterResultPair(  s_q_max  , "DOSE", tr(" %1 %2").arg(s_q_max, drug_qu_unit) ,
                                                             id       , "CIP", drugName,
                                                             tr("Dose maximum %1 trop elevée <br/>").arg(s_q_max+" "+drug_qu_unit),"IT_DOSE", "4"
                                                          )
                                       );
       ...
       { interResultPairList.append( C_InterResultPair(  s_q_max  , "DOSE", tr("Contrôle des doses") ,
                                                         id       , "CIP", drugName,
                                                         tr("Pas de données pour le contrôle des doses <br/>"),"IT_DOSE", "1"
                                                      )
                                   );
 ...
 */
/*
 - Le nouveau-né, de la naissance à 21 jours
 - Le nourrisson, de 21 jours à 2 ans.
*/
     ...
     while ( C_GET_THE_FCO_ID_BY_SPE.next( s_codeId  ) )
           { while ( C_GET_THE_DET_FCO.next( C_GET_THE_FCO_ID_BY_SPE.IDFCO()  ) )
                   { if (C_GET_THE_DET_FCO.GRP()=="01" )
                     ...
                     if (C_GET_THE_DET_FCO.GRP()=="02" )        // niveau de gravité
             ...
             interResultPairList.append( C_InterResultPair( ""      , "VIGIL",  tr("Effets sur l'aptitude à conduire des véhicules et à utiliser des machines"),
                                                            cip     , "CIP",    drugName,
                                                            tr("Effets sur l'aptitude à conduire des véhicules et à utiliser des machines : <br/>")+txt,"IT_VIG",
                ...
                while ( C_GET_THE_DET_POSO_SPE.next(numFic , 2  ) )
                      {
                        ...
                        //--- durée minimum et durée maximum                    1.N°.2.2
                        //--- commentaire durée                                 1.N°.3.IPOCOD_NUM_ORD_PK
                        ...
                        if (grp[0]=='1')
                           {
                            if (cur_grp != -1 && cur_grp != grp.toLatin1()[2])
                             ...
                             typ_grp = grp.toAscii()[4];
                             sub_grp = grp.toAscii()[6];
                             ...
                             if (typ_grp=='2')          // --- frequence
                                {
                                 ...
                                 if (sub_grp=='2')      // --- durée minimum et durée maximum        Durée de traitement max
                                    { min_duree  = min;            // 10
                                      max_duree  = max;            // 30
                                            ...
                                            )
{
 ...
 QString  ret   = "";
           ...
           if (min_poso.length() && max_poso.length()) gramar += "<gpp>[" + min_poso + "-" + max_poso+";{{MOMENT}}]" ; //result += " de " + min + tr(" à ") + max + " " + C_GET_THE_DET_POSO_SPE.INFO_02();
...
{   QString    ret  = "";
    ...
    QString s_emba  = "";
    QString s_prix  = getPrixUCDFromCIP(cip);            // es tu là ?
 ...
 {   QList<C_CompositionItem> retList;
     ...
     while ( C_GET_THE_SUB_SPE.next( codeId , 2  ) )
           {QString qu = C_GET_THE_SUB_SPE.DOSESUBST();
            ...
            //                    C_GET_THE_SUB_SPE.DOSESUBST() 2eme tour  -->  exprimé (e) en MORPHINE BASE ANHYDRE 1 MG
            //        exple AUGMENTIN NUMORDRE() == 1
            //                    C_GET_THE_SUB_SPE.DOSESUBST() 1er  tour -->   NON RENSEIGNE
            //                    C_GET_THE_SUB_SPE.DOSESUBST() 2eme tour  -->  exprimé (e) en AMOXICILLINE ANHYDRE 100 MG
            ...
            if (C_GET_THE_SUB_SPE.DOSESUBST() != "NON RENSEIGNE" &&  // si NON RENSEIGNE esperons qu'une autre forme valide sorte au prochain tour
               ...
               {
                  retList.append ( C_CompositionItem( C_GET_THE_SUB_SPE.LIBSUBST().remove("exprimé(e) en "),
                                                      qu,
                                                      C_GET_THE_SUB_SPE.UDOSESUBST(),
                                                      C_GET_THE_SUB_SPE.UDOSESUBST()+"(s)"
                                                     )
                  /*
                                   C_InterResultPair(qu,  C_GET_THE_SUB_SPE.UDOSESUBST()       ,       C_GET_THE_SUB_SPE.LIBSUBST().remove("exprimé(e) en "),
          ...
          while ( C_GET_THE_SUB_SPE.next( codeId , 2  ) )
                {//........................ theriaque peut renvoyer la variete de situations suivantes ..........................
                 ...
                 if (C_GET_THE_SUB_SPE.DOSESUBST() != "NON RENSEIGNE" &&  // si NON RENSEIGNE esperons qu'une autre forme valide sorte au prochain tour
                    ...
                    {
                       retList.append ( C_GET_THE_SUB_SPE.LIBSUBST().remove("exprimé(e) en ") );
                       lastNumOrdre = C_GET_THE_SUB_SPE.NUMORDRE();
                    }
                }
         return retList;
...
{   Q_UNUSED(dataMustBeReadInCache);
 ...
 /*
  ...
  Effets indésirables.*
  */
 QString javaS_CreatePanel_Body =  "<script type=\"text/javascript\">\n"             // ce bloc contient les creations des panneaux et est à la fin
                                   "<!--\n";
 QString path                   = ".";
 QString block                  = ".";
 QString tmp                    = "";
     ...
     while ( C_GET_THE_SPECIALITE.next (  cip ) )    // results cursor
            ...
            if (C_GET_THE_SPECIALITE.SP_TYPE_SPE()=="G"||C_GET_THE_SPECIALITE.SP_TYPE_SPE()=="R")
               {C_GET_THE_GEN_SPE_CUR  C_GET_THE_GEN_SPE ( database(), this );
                ...
                if (C_GET_THE_SPECIALITE.SP_TYPE_SPE()=="G")
                   { str +=   makeLigneGenerique(C_GET_THE_SPECIALITE.SP_NOM(),cip );    // on le rentre là car parfois pas dans liste afssaps
                   }
                ...
                //,,,,,,,,,,,,,, si referent non valide aller à la pêche .............
                ...
                while ( C_GET_THE_GEN_SPE.next(codeId , 2 )  ) // -- 2 -  generiques affsapps
                      {if (C_GET_THE_GEN_SPE.MY_GSP_NOM().endsWith("NSFP")) continue;   // ne pas retenir les NeSeFaitPlus
                       if (dum.length()==0 && C_GET_THE_GEN_SPE.GEN_REF() == C_GET_THE_GEN_SPE.SP_CODE())
                          { tmp  =  C_GET_THE_GEN_SPE.MY_GSP_NOM() + " (réf. Ansm/Afssaps)";         //  Plan A relever le referent pour le titre
                            dum  =  C_GET_THE_GEN_SPE.PRE_CODE();
                          }
                       if (dum.length()==0 && C_GET_THE_GEN_SPE.GEN_TYPE()=="R" )
                          { tmp  =  C_GET_THE_GEN_SPE.MY_GSP_NOM() + " (réf. Ansm/Afssaps)";
                            dum  =  C_GET_THE_GEN_SPE.PRE_CODE();
                          }
                       if (isResult==0) block += "<TABLE BORDER=\"1px\"; width=\"100%\">             "
                                                 "<TR>                             "
                                                 "<TH align=\"left\"; width=\"45%\";> Spécialités génériques Afssaps </TH>  "
                ...
                //           on prend le dernier Theriaque trouvé (forcément sans prix)
                if (dum.length()==0)
                   {tmp     =   last_gen + " (réf. Thériaque)";
                    dum     =   last_cip;
                    str     +=  makeLigneGenerique(tmp , dum );
                   }
                if (str.length())
                ...
                if (block.length())
                   {html  += addCollapsibleJSPanel       ( indexCollapsiblePanel++,
                                                          ...
                                                          "",
                                                          tr("Référent générique : %1<br/>Cip %2 ").arg(tmp,dum),
                                                          "",                                        // cim 10
                                                          " ",
                                                          path,   //Réf. : Journal Officiel du
                                                          block ,
            ...
            while ( C_GET_THE_SUB_SPE.next(codeId ,1  ))
                  {int isNotEffetNotoire = 1;
                   while ( C_GET_THE_EXP_ID.next(C_GET_THE_SUB_SPE.CODESUBST()))
                      {if (isResult==0) block += tr("<br/><br/><b><u>Exipients à effets notoires.</b></u>");
                       block += addCollapsibleJSPanel       ( indexCollapsiblePanel++,
                                                             ...
                                                             .replace(">","&gt;").replace("<","&lt;")
                                                             .replace("- Dénomination",              "<br/>° <b><u>Dénomination</u></b>")
                                                             ...
                                                             .replace("Réf. : Journal Officiel",     "<br/>° <b><u>Réf. : Journal Officiel</u></b>")
                                                             ,
                                                             javaS_CreatePanel_Body,
                                                             "panel_EffeNotoireCss");
                       ++isResult;
                   ...
                   if (isNotEffetNotoire)  // si pasd'effet notoire affichage ligne simple
            ...
            */
            ...
            while ( C_GET_MARCHE_PUBLIC_SPE.next( s_codeId ) )
                  { block += tr("<br/><b><u>Nomenclature du code des marchés publics</b></u> : %1").arg(C_GET_MARCHE_PUBLIC_SPE.CMP_LIBELLE());
                  }
            ...
            while ( C_GET_AGR_COLLECT_FROM_CIP.next( cip) )
                  { if ( C_GET_AGR_COLLECT_FROM_CIP.AGRCOLL()=="O")
                       { block += tr("<br/><b><u>Agrément aux collectivités</u> : </b> Date JO ")
            ...
            while ( C_GET_RES_HOP_FROM_CIP.next( cip) )
                  { block += tr("<br/><b><u> Reservé à l'hopital</u> : </b>  ") + C_GET_RES_HOP_FROM_CIP.RESULT();
                  }

            QString remb = "";     //....................  CONDITIONS DE PRESCRIPTION, DE DÉLIVRANCE REMBOURSEMENT.........
            isResult     = 0;
            ...
            while ( (isResult=C_GET_THE_SPE_STATUT.next( codeId  ) ) )
                  {if (isResult>=2)
                      {if (isResult==2) block += tr("<br/><b><u>Condition de prescription et de délivrance.</u></b>");
                       block += "<br/>&nbsp; &nbsp; &nbsp;";
                       block += C_GET_THE_SPE_STATUT.INFO_01().replace(">","&gt;").replace("<","&lt;").replace("- Décision","<br/>- Décision").replace("- Arrêté","<br/>- Arrêté").replace("*","<br/>° ")
            ...
            while ( C_GET_THE_LAB_SPE.next(codeId ,1  ) )
                  {block += tr("<br/><b><u>Établissement détenteur de l'AMM</u> : </b>%1") .arg(C_GET_THE_LAB_SPE.NomLab()) +  tr("  date : %1") .arg(C_GET_THE_LAB_SPE.DateRef().toString("dd-MM-yyyy"));;
                  }
            while ( C_GET_THE_LAB_SPE.next(codeId ,2  ) )
                  {block += tr("<br/><b><u>Établissement exploitant</u> : </b>%1") .arg(C_GET_THE_LAB_SPE.NomLab()) +  tr("  date : %1") .arg(C_GET_THE_LAB_SPE.DateRef().toString("dd-MM-yyyy"));;
                  }
            ...
            while ( (isResult=C_GET_THE_ATC_DDD.next( C_GET_THE_SPECIALITE.SP_CATC_CODE_FK(), 1)) )
                  { if (isResult==1) block += tr("<b><u>Dose journalière conseillée pour la classe ATC</u> : (%1)<br/></b>").arg(C_GET_THE_SPECIALITE.SP_CATC_CODE_FK());
            ...
            while ( (isResult=C_GET_THE_ATC_DDD.next( QString::number(C_GET_THE_SPECIALITE.SP_CODE_SQ_PK()), 2)) )
                 { if (isResult==1) block += tr("<br/><b><u>Dose journalière conseillée pour cette spécialité.<br/></b></u>");
                   block += trUtf8("<b>&nbsp; &nbsp; &nbsp;</b>  ")             + //C_GET_THE_ATC_DDD.CATC()    +
                                                              trUtf8(" voie   : ")      + C_GET_THE_ATC_DDD.NOMVOIE() +
                                                              trUtf8(" ")               + C_GET_THE_ATC_DDD.DOSAGE()  +
                                                              trUtf8(" %1 par jour").arg( C_GET_THE_ATC_DDD.NOMUNITE() );
                                                                   ...
                                                                   int &indexCollapsiblePanel , const QString & path , QString &javaS_CreatePanel_Body)
{
    ...
    while (C_GET_THE_CHOIX.next( s_codeId , 3 ) )  // 'AFS_ANA' AGENCE FRANÇAISE DE SECURITE SANITAIRE DES PRODUITS DE SANTE et conf consensus RECOMMANDATIONS DE BONNE PRATIQUE PREVENTION
          { while (C_GET_THE_DOC_SPE.next( C_GET_THE_CHOIX.NU_FICHE() , 3 ) )
              { if (isResult==0) block += tr("<br/><b><u>Recommandations et protocoles thérapeutiques Afssaps, HAS, INCa<br/></b></u>");
                block += addCollapsibleJSPanel       ( indexCollapsiblePanel++,
                                                      C_GET_THE_CHOIX.DATE_TEXTE().toString("dd-MM-yyyy"),
                                                      C_GET_THE_CHOIX.PRECISION1(),
                                                      C_GET_THE_CHOIX.TITRE(),
                                                      "",                                        // cim 10
                                                      tr("Recommandations et protocoles thérapeutiques Afssaps, HAS, INCa"),
                                                      path,
                                                      C_GET_THE_DOC_SPE.TEXTE().replace(">","&gt;").replace("<","&lt;").replace(" -","<br/> °"),
                                                      javaS_CreatePanel_Body );
                ++isResult;
    ...
    while (C_GET_THE_CHOIX.next( s_codeId , 4 ) )  // 'RAPPE'  RAPPORT PUBLIC D'EVALUATION LOVENOX
          { while (C_GET_THE_DOC_SPE.next( C_GET_THE_CHOIX.NU_FICHE() , 4 ) )
              { if (isResult==0) block += tr("<br/><b><u>Rapports publics d'évaluation.<br/></b></u>");
                block += addCollapsibleJSPanel       ( indexCollapsiblePanel++,
                                                      C_GET_THE_CHOIX.DATE_TEXTE().toString("dd-MM-yyyy"),
                                                      C_GET_THE_CHOIX.PRECISION1(),
                                                      C_GET_THE_CHOIX.TITRE(),
                                                      "",                                        // cim 10
                                                      tr("Rapport public d'évaluation"),
                                                      path,
                                                      C_GET_THE_DOC_SPE.TEXTE().replace(">","&gt;").replace("<","&lt;").replace(" -","<br/> °"),
                                                      javaS_CreatePanel_Body );
                ++isResult;
    ...
    while (C_GET_THE_CHOIX.next( s_codeId , 5 ) )  // C_PAT' CONSEILS/EDUCATION SANITAIRE CONSEILS au aptient
          { while ( C_GET_THE_DOC_SPE.next( C_GET_THE_CHOIX.NU_FICHE() , 5 ) )
              { if (isResult==0) block += tr("<br/><b><u>Conseils au patient, éducation sanitaire.<br/></b></u>");
                block += addCollapsibleJSPanel       ( indexCollapsiblePanel++,
                                                      C_GET_THE_CHOIX.DATE_TEXTE().toString("dd-MM-yyyy"),
                                                      C_GET_THE_CHOIX.PRECISION1(),
                                                      C_GET_THE_CHOIX.TITRE(),
                                                      "",                                        // cim 10
                                                      tr("Conseils au patient, éducation sanitaire"),
                                                      path,
                                                      C_GET_THE_DOC_SPE.TEXTE().replace(">","&gt;").replace("<","&lt;").replace(" -","<br/> °"),
                                                      javaS_CreatePanel_Body );
                ++isResult;
    ...
    while ( C_GET_THE_POSO.next(  s_codeId  ) )
          { type = C_GET_THE_POSO.TYP();
            ...
            { case 0: numFic    = C_GET_THE_POSO.MY_NOFIC();                                   break;
              case 1: voieAd    = C_GET_THE_POSO.MY_INFO_01();                                 break;
              case 2: terrain  += C_GET_THE_POSO.MY_INFO_01()+", ";                            break;
              case 3: {natIndic += "°&nbsp;&nbsp;&nbsp;"+C_GET_THE_POSO.MY_INFO_01()+"<br/>";
                      if (addTerr==0)
                         {terrain += " <br/>(";
                          ++addTerr;
                         }
              ...
              case 4: cpmIndic += "°&nbsp;&nbsp;&nbsp;"+C_GET_THE_POSO.MY_INFO_01()+"<br/>";    break;
            }
            if (type==5)
               {if (terrain.length() && addTerr) terrain += ")";
                if (natIndic.length())
                           ...
                           { cur_grp = grp.toAscii()[2];
                             ...
                             if (typ_grp=='1')          // --- minimum et maximum de la posologie
                                {result += tr("<h2 class=titre_resume>Posologie %1</h2>").arg( C_GET_THE_DET_POSO_SPE.NATURE()+" "+cplNat );
                                 if (min.length() && max.length()) result += " de " + min + tr(" à ") + max + " " + C_GET_THE_DET_POSO_SPE.INFO_02();
                             ...
                             if (typ_grp=='2')          // --- frequence
                                {
                                 if (sub_grp=='0')      // --- frequence minimum et maximum
                                    { result += tr("<h2 class=titre_resume>Fréquence</h2> %1").arg( C_GET_THE_DET_POSO_SPE.NATURE() );
                                      if (min.length() && max.length()) result += " de " + min + " " + C_GET_THE_DET_POSO_SPE.INFO_02() + tr(" à ") + max + " " + C_GET_THE_DET_POSO_SPE.INFO_03() + " ";
                                      else if (min.length())            result += min + " " + C_GET_THE_DET_POSO_SPE.INFO_02() + " ";
                                 ...
                                 if (sub_grp=='2')      // --- durée minimum et durée maximum        Durée de traitement max
                                    { result += tr("<h2 class=titre_resume>Durée de traitement</h2> %1").arg( C_GET_THE_DET_POSO_SPE.NATURE() );
                                      if (min.length() && max.length()) result += " de " + min + " " + C_GET_THE_DET_POSO_SPE.INFO_02() + tr(" à ") + max + " " + C_GET_THE_DET_POSO_SPE.INFO_03() + " ";
                                      else if (min.length())            result += min + " " + C_GET_THE_DET_POSO_SPE.INFO_02() + " ";
                                      else if (max.length())            result += max + " " + C_GET_THE_DET_POSO_SPE.INFO_03() + " ";
                                      if (C_GET_THE_DET_POSO_SPE.INFO_01().length()) result += "<br/>" + C_GET_THE_DET_POSO_SPE.INFO_01();
                             ...
                             if (typ_grp=='3')          // --- commentaire durée
                                {result += "<br/>" + C_GET_THE_DET_POSO_SPE.INFO_01();
                                }
                             last_grp = cur_grp;
                           }
                                                 ...
                                                 int &indexCollapsiblePanel , const QString & path , QString &javaS_CreatePanel_Body)
{
    ...
    if (block.length()) html  += tr("<p class=nom_chapitres>EFFETS INDÉSIRABLES À DOSES THÉRAPEUTIQUES (Cliniques)</p> %1") .arg(block);
    block           =   makeEffetsIndesirables_subBlock( codeId, 2,  indexCollapsiblePanel, path,javaS_CreatePanel_Body);
    if (block.length()) html  += tr("<p class=nom_chapitres>EFFETS INDÉSIRABLES À DOSES THÉRAPEUTIQUES (Paracliniques)</p> %1") .arg(block);
    block           =   makeEffetsIndesirablesSurdosage_subBlock( codeId, 3,  indexCollapsiblePanel, path,javaS_CreatePanel_Body);
    if (block.length()) html  += tr("<p class=nom_chapitres>EFFETS INDÉSIRABLES EN CAS DE SURDOSAGE (Cliniques)</p> %1") .arg(block);
    block           =   makeEffetsIndesirablesSurdosage_subBlock( codeId, 4,  indexCollapsiblePanel, path,javaS_CreatePanel_Body);
    if (block.length()) html  += tr("<p class=nom_chapitres>EFFETS INDÉSIRABLES EN CAS DE SURDOSAGE (Paracliniques)</p> %1") .arg(block);
...
{   int isResult         = 0;
    ...
    */
    ...
    } else if (  TypId == 4) {    // -- texte fréquence
    ...
    } else if (  TypId == 11) {   // -- conduite à tenir
    } else if (  TypId == 12) {   // -- commentaire dose thérapeutique
    } else if (  TypId == 13) {   // -- texte Nature(s) EI
    } else if (  TypId == 14) {   // -- texte conduite à tenir
    } else if (  TypId == 15) {   // -- texte conduite à tenir
    ...
    while ( C_GET_THE_EFFIND_SPE.next( codeId , type  ) )       //   1/clinique   2/paraclinique 3/
          { terrFav      = "";
            ...
            while ( (isResult = C_GET_THE_DET_EFFIND.next( C_GET_THE_EFFIND_SPE.CODEEFFIND() , 4  )) )
                  { // str += tr("<br/><b>Texte fréquence : </b>  ") + C_GET_THE_DET_EFFIND.LIBIND();
            ...
            while ( (isResult = C_GET_THE_DET_EFFIND.next( C_GET_THE_EFFIND_SPE.CODEEFFIND() , 11  )) )
                  { // str += tr("<br/><b>conduite à tenir : </b>  ") + C_GET_THE_DET_EFFIND.LIBIND();
            ...
            while ( (isResult = C_GET_THE_DET_EFFIND.next( C_GET_THE_EFFIND_SPE.CODEEFFIND() , 12  )) )
                  { // str += tr("<br/><b>commentaire dose thérapeutique : </b>  ") + C_GET_THE_DET_EFFIND.LIBIND();
            ...
            while ( (isResult = C_GET_THE_DET_EFFIND.next( C_GET_THE_EFFIND_SPE.CODEEFFIND() , 14  )) )
                  { // str += tr("<br/><b>texte conduite à tenir : </b>  ") + C_GET_THE_DET_EFFIND.LIBIND();
            ...
            if (fceText.length())     str += tr("<br/><b>Commentaire fréquence : </b>  ")             + fceText;
            ...
            if (catLibelle.length())  str += tr("<br/><b>Conduite à tenir : </b>  ")                  + catLibelle;
            if (commDoseThe.length()) str += tr("<br/><b>Commentaire dose thérapeutique : </b>  ")    + commDoseThe;
            if (natuTxt.length())     str += tr("<br/><b>Commentaire effets indésirables : </b>  ")   + natuTxt;
            if (cat1.length())        str += tr("<br/><b>Commentaire conduite à tenir 1 : </b>  ")    + cat1;
            if (cat2.length())        str += tr("<br/><b>Commentaire conduite à tenir 2 : </b>  ")    + cat2;
            if (str.length())
               { block += addCollapsibleJSPanel       (             indexCollapsiblePanel++,
                                                                    date,
...
{   int isResult         = 0;
    ...
    */
    /*
     ...
     // 6 -- conduite à tenir nature du surdosage
     // 7 -- texte nature du surdosage
     // 8 -- texte conduite à tenir
    ...
    while ( C_GET_THE_EFFIND_SPE.next( codeId , type  ) )       //   1/clinique   2/paraclinique 3/
          {
            ...
            if (chimClass.length())     str += tr("<br/><b>Classe chimique : </b>  ")                                + chimClass;
            if (pharClass.length())     str += tr("<br/><b>Classe pharmaco thérapeutique : </b>  ")                  + pharClass;
            ...
            if (catNatSur.length())     str += tr("<br/><b>Conduite à tenir selon nature du surdosage: </b>  ")      + catNatSur;
            if (natuTxt.length())       str += tr("<br/><b>Commentaire nature du surdosage : </b>  ")                + natuTxt;
            if (catText.length())       str += tr("<br/><b>Commentaire conduite à tenir : </b>  ")                   + catText;
            if (str.length())
               { block += addCollapsibleJSPanel       (             indexCollapsiblePanel++,
                                                                    date,
                                                                    refOff,                            // reference officielle
      ...
      { //............................ Mode d'administration ..........................................
        ...
        if (str.length())
           { block += addCollapsibleJSPanel       (             indexCollapsiblePanel++,
                                                                date,
                                                                refOff,                          // reference officielle
                                                                tr("Recommandation utilisation,manipulation et élimination"),     //
                                                                titre,                           // Passage transplacentaire
                                                                "",
                                                                path,
                                                                str,
...
{   QString block    = "";
    ...
    while ( C_GET_THE_INTER_SPE.next( CodeId  ) )
          { block +=  C_GET_THE_INTER_SPE.TEXTINTER().replace("* ","<br/><br/> * ").replace(" - ","<br/> - ").replace("+ "," <br/><br/> +") + "<br/>";
            block +=  tr("<b> Réf. officielle : </b>  ") + C_GET_THE_INTER_SPE.REFOFF() + " en date du " + C_GET_THE_INTER_SPE.DTREF("dd-MM-yyyy") + "<br/>";
          }
...
{   QString block    = "";
    ...
    while ( C_GET_THE_PRE_CSV.next( cip , 1  ) )
          { if (C_GET_THE_PRE_CSV.TYPE_PRE_CSV()=="11")
               { block += tr("<b> Durée de conservation : </b> %1 <br/>").arg( QString::number(C_GET_THE_PRE_CSV.VALEUR_1()) + " " +C_GET_THE_PRE_CSV.INFO_1() );
               }
            if (C_GET_THE_PRE_CSV.TYPE_PRE_CSV()=="211")
               { block += tr("<b> Température : </b> %1 <br/>").arg(C_GET_THE_PRE_CSV.INFO_1());
               }
            if (C_GET_THE_PRE_CSV.TYPE_PRE_CSV()=="3")
               { block +=  tr(" %1<br/>").arg(C_GET_THE_PRE_CSV.TEXTE());
               }
            if (C_GET_THE_PRE_CSV.TYPE_PRE_CSV()=="4")
               { block +=  tr("<b> Réf. Officielle : </b>  %1<br/>").arg(C_GET_THE_PRE_CSV.INFO_1() + " en date du " + C_GET_THE_PRE_CSV.DATE_1("dd-MM-yyyy"));
               }
          }
...
{   QString block    = "";
    ...
    while ( C_GET_THE_DET_PHDYNA.next( codeId, 2  ) )
          { block +=    tr("<br/><b>Réf. Officielle</b> : ") + C_GET_THE_DET_PHDYNA.TEXTEPH()
                      + tr(" En date du : ") + C_GET_THE_DET_PHDYNA.DTREF("dd-MM-yyyy");
          }

    if (block.length()) html  += tr("<p class=nom_chapitres> Propriétés pharmacodynamiques </p> %1") .arg(block);
    return block;
}
...
{    QString block    = "";
    ...
    */
    ...
    if (block.length()) html  += tr("<p class=nom_chapitres> Effets sur l'aptitude à conduire des véhicules et à utiliser des machines </p> %1") .arg(block);
    return block;
}
//------------------------------------ makeCinetiqueBlock -------------------------------
...
{   QString block    = "";
    ...
    while ( C_GET_THE_CINETIQUE_SPE.next( codeId  ) )
          { block +=    C_GET_THE_CINETIQUE_SPE.PHARMACOTEXT().replace("* ","<br/><br/> * ")
                      + tr("<br/><b>Réf. Officielle</b> : ") + C_GET_THE_CINETIQUE_SPE.REFOFFICIELLE()
                      + tr(" En date du : ") + C_GET_THE_CINETIQUE_SPE.DTREF("dd-MM-yyyy");
          }

    if (block.length()) html  += tr("<p class=nom_chapitres> Propriétés pharmacocinétiques </p> %1") .arg(block);
    return block;
}
    ...
    while ( C_GET_THE_SECUPRECLINIQUE_SPE.next( codeId  ) )
          { block += C_GET_THE_SECUPRECLINIQUE_SPE.MY_TEXTFIC();
            while ( C_GET_THE_REF_SECUPRECLINIQUE.next( codeId , C_GET_THE_SECUPRECLINIQUE_SPE.MY_CODEFIC()  ) )
                  { block += tr("<br/><b>Réf. Officielle</b> : ") + C_GET_THE_REF_SECUPRECLINIQUE.MY_REFOFF()
                             + tr(" En date du : ") + C_GET_THE_REF_SECUPRECLINIQUE.MY_DATEREF("dd-MM-yyyy");
                  }
          }
    if (block.length()) html  += tr("<p class=nom_chapitres> Sécurité préclinique </p> %1") .arg(block);
    return block;
}
...
{   QString block    = "";
    ...
    while ( C_GET_THE_GR_FIC_SPE.next( codeId  ) )
          {
            ...
            if ( C_GET_THE_GR_FIC_SPE.FIXPROT()[0]=='O' )
               { block += tr("<b><u>Fixation protéique</u> : </b> Oui <br/>");
               }
            if ( C_GET_THE_GR_FIC_SPE.FIXPROT()[0]=='N' )
               { block += tr("<b><u>Fixation protéique</u> : </b> Non <br/>");
               }
            ...
            while ( C_GET_THE_GR_SPE.next( codeId ,1 , C_GET_THE_GR_FIC_SPE.MY_CODE_FICHE()  ) )   // inutile
                  { str += tr("<br/><b> Informations générales de la fiche : Id theriaque </b>  ") + C_GET_THE_GR_SPE.TEXTGR();
                  }
             */
            ...
            while ( C_GET_THE_GR_SPE.next( codeId ,2 , C_GET_THE_GR_FIC_SPE.MY_CODE_FICHE()  ) ) // Observations
                  { str += tr("<br/><b> Données expérimentales animales : </b>  ") + C_GET_THE_GR_SPE.TEXTGR();
                  }
            ...
            while ( C_GET_THE_GR_SPE.next( codeId ,6 , C_GET_THE_GR_FIC_SPE.MY_CODE_FICHE()  ) ) // Observations
                  { str += tr("<br/><b> Données cliniques humaines :  </b>  ") + C_GET_THE_GR_SPE.TEXTGR();
                  }
            ...
            while ( C_GET_THE_GR_SPE.next( codeId ,11 , C_GET_THE_GR_FIC_SPE.MY_CODE_FICHE()  ) ) // Niveau de risque
                  { str += tr("<br/><b> Recommandations et conduite à tenirs (niveau de risque) : </b>  ") + C_GET_THE_GR_SPE.TEXTGR();
                  }
            ...
            while ( C_GET_THE_GR_SPE.next( codeId ,13 , C_GET_THE_GR_FIC_SPE.MY_CODE_FICHE()  ) )  // conduite a tenir
                  { str += tr("<br/><b> Recommandations et conduite à tenir chez la femme enceinte : </b>  ") + C_GET_THE_GR_SPE.TEXTGR();
                  }
            ...
            while ( C_GET_THE_GR_SPE.next( codeId ,15 , C_GET_THE_GR_FIC_SPE.MY_CODE_FICHE()  ) )  // conduite a tenir
                  { str += tr("<br/><b> Recommandations et conduite à tenir chez le nouveau né : </b>  ") + C_GET_THE_GR_SPE.TEXTGR();
                  }
    ...
    while ( C_GET_THE_AL_FIC_SPE.next( codeId  ) )
          ...
          if ( C_GET_THE_AL_FIC_SPE.FIXPROT()[0]=='O' )
             { str += tr("<b><u>Fixation protéique</u> : </b> Oui <br/>");
             }
          if ( C_GET_THE_AL_FIC_SPE.FIXPROT()[0]=='N' )
             { str += tr("<b><u>Fixation protéique</u> : </b> Non <br/>");
             }
          ...
          if ( C_GET_THE_AL_FIC_SPE.CONCENT()[0]=='N' )
           ...
           while ( C_GET_THE_AL_SPE.next( codeId , 1 , C_GET_THE_AL_FIC_SPE.CODE_FICHE()  ) )
                 { str += tr("<br/><b> Conduite à tenir : </b>  ") + C_GET_THE_AL_SPE.TEXTAL();
                 }
           ...
           while ( C_GET_THE_AL_SPE.next( codeId , 8 , C_GET_THE_AL_FIC_SPE.CODE_FICHE()  ) )
                 { str += tr("<br/><b> Observations chez le nouveau-né : </b>  ") + C_GET_THE_AL_SPE.TEXTAL();
                 }
          }
    ...
    if (block.length()) html  += tr("<p class=nom_chapitres> Grossesse Allaitement Procréation </p> %1") .arg(block);
    return block;
}
//------------------------------------ makeCIBlock -------------------------------
...
{   QString block       = "";
    C_GET_CIPEMG_SPE_CUR          C_GET_CIPEMG_SPE         ( database(), this );     // retirer les CI
    C_GET_THE_DET_CIPEMG_CUR      C_GET_THE_DET_CIPEMG     ( database(), this );     // extraire les info
    C_GET_THE_REF_CIPEMG_CUR      C_GET_THE_REF_CIPEMG     ( database(), this );     // extraire les dates et amm concernés
    QString niveau = "";
    //........... Extraction des CI niveau 'type'  pour la specialite 's_codeId' ............
    ...
    niveau = codeNatureCi_to_Text(QString(type).remove('\'').toAscii()[0]);
...
{       QString idStr = QString::number(index);
        ...
        descriptif += "<br/>";
        descriptif  = C_Utils_Html::makeLinkList(descriptif,"<br/>","EOL", idStr).replace("- ","<br/>&nbsp;&nbsp;&nbsp;°");
 ...
 *  \return QList<C_InterResultPair> result interaction pair list
 */
QList<C_InterResultPair> C_BDM_TheriaquePlugin::getInteractionRedundancy( const QStringList &cipList)
{Q_UNUSED(cipList);
 QList<C_InterResultPair> interResultPairList;
 return interResultPairList;
}
...
{Q_UNUSED(filter);
 ...
 QString libelleSMR   = "";
 QString descriptif   = "";
 QMap  <QString, C_BDM_IndicationRecord> mapResult;        // on mappe pour eviter les doublons de codes rendus en cas de descriptifs différents...
...
//                   therLevel             MT level
// SMR majeur                  80   à 99   90
// SMR important         2     60   à 79   70
// SMR Modéré            4     40   à 59   50
// SMR Faible            1     20   à 39   30
// SMR insuffisant       3     1    à 19   10
// SMR non précisé       0                  0
...
// service médical rendu  SMR
//                   therLevel             MT level
// SMR majeur                  80   à 99   90    / 20 -> 4
// SMR important         2     60   à 79   70    / 20 -> 3
// SMR Modéré            4     40   à 59   50    / 20 -> 2
// SMR Faible            1     20   à 39   30    / 20 -> 1
// SMR insuffisant       3     1    à 19   10    / 20 -> 0
// SMR non précisé       0                  0
/*
...
{   QStringList s_libelle;
    ...
    static int  s_asmrLevel[7] = {78,                           62,               46,                  30,                    24,                             8,                0};
                s_libelle  <<tr("SMR non précis\303\251")<<tr("SMR faible")<<tr("SMR important")<<tr("SMR insuffisant")<<tr("SMR mod\303\251r\303\251")<<tr("SMR majeur")<<tr("SMR difficile à pr\303\251ciser");
  //          0                   1            2           3            4                 5          6
  //          date                id_1 (cip)   libelle_1   id_2 (cimx)  libelle_2         descrip_2  therLevel
  // " SELECT FINSMR_DATE_SMR_PK, PRE_CODE_PK, SP_NOMLONG, CIM_CODE_PK, CIM_LIBELLE_LONG, FIN_TEXTE, FINSMR_CDF_SMR_CODE_FK_PK "
