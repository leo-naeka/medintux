...
{ return getCalculatedTabZone_H(buttonH, marge) + buttonH*3 + marge*7 + getCalculatedSequenceH( buttonH, marge) + nb_item * item_h;
}
...
public slots:
    ...
    void Slot_editDateTimeChanged(const QDateTime& dateTime);
...
private:
    ...
    QDateTimeEdit       *m_pQDateTimeEdit;
    CMyColorButton      *m_Button_QSP;
    int                  m_marge;
    int                  m_duree;
    int                  m_dureeUnit;
    QFont                m_fGrd;
...
public:
    ...
    ~C_Frm_Sequence(){}
    int   sequenceToInterface(const QString &sequence);
    int   sequenceTimeToInterface(const QString &sequence);
    int   sequenceHoursToInterface(const QString &before, const QString &after, const QString &value);
    void  sequenceDaysToInterface(const QString &sequence);
    QString interfaceToGrammar(QString &forme);
    ...
    void    controlMenuCheckBox(int idMenu);
    ...
    QString adjustDecPartString(float f_quantite, float divide);
...
public:
    ...
    ~C_Wdg_Stack_Sequence(){}
     QString         interfaceToGrammar( QString &formesList );
...
public:
 ...
 private:
     C_Frm_Produit   *c_frm_produit();      // TODO dependance à virer
     C_Frm_Posologie *c_frm_posologie();    // TODO dependance à virer
     ...
     int                isExistsThisHit( QString  grammar);
...
private:
  ...
  C_BitMapCollection      *m_pC_BitMapCollection;
public :
  QPointer<C_Frm_PosologieHitList>  m_HitList;
...
public:
    ...
    void                     displayInfo(const  QString &info, const  QString &title, int x, int y , int w, int h);
    QString                  serialize(const QString &ofset = "");

    QRect                    get_displayInteractionWindowGeometry();
    void                     set_Thread_DBM_Plugin(const QString &_namePlugin);
