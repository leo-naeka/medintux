    ...
    while (!file.atEnd())
          {
           ...
           if (posInterRes==-1) posInterRes = line.indexOf("Précaution d'emploi");
...
{int nb                                       = 0;
 ...
 if (pk_interactant.length()==0)
    {err = tr("ERREUR : C_BaseMedica::createInteractionForThisComposant()\n pas de composant trouvé pour cette valeur '%1' ").arg(libelleInteractant).toAscii();
     outMessage(err);
     return nb;
    }
...
{   //....................... ouvrir le fichier en mode QTextStream ...........................
    if ( !QFile::exists( fileName ) )
       {outMessage(  tr("ERREUR : Ce fichier : '%1' n'existe pas.").arg(fileName).toAscii(), __FILE__, __LINE__);
        return 0;
       }
...
{QStringList fieldsList;
 ...
 //....................... ouvrir le fichier en mode QTextStream ...........................
 if ( !QFile::exists( fileName ) )
    {outMessage( tr("ERREUR : initAfssapsDataBase() Ce fichier : '%1' n'existe pas.").arg(fileName).toAscii(), __FILE__, __LINE__);
                                                        return 0;
    }
 QFile file( fileName );
 if ( !file.open( QIODevice::ReadOnly  ) )
    {outMessage( tr("ERREUR : initAfssapsDataBase() Ce fichier : '%1' n'est pas accessible en lecture.").arg(fileName).toAscii(), __FILE__, __LINE__);
                                                        return 0;
    }
 ...
 int nb = record.count();
 int  i = 0;
 outMessage( tr("----------- liste des champs de la table : '%1' ---------------").arg(tableName).toAscii());
 for (i = 0; i<nb; ++i)
 ...
 if (fieldsList.count()==0)
    {err = tr("Les champs de cette table : '%1.%2' n'existent pas ou sont non accessibles..").arg(database().databaseName(),tableName).toAscii();
     outMessage(  err, __FILE__, __LINE__);
     file.close();                                      return 0;
    }
 //.................... effacer la table ....................................................
 ...
 for ( i=0; i<nb; ++i)
     {            fieldName        = fieldsList[i];
      ...
      err = tr("table : '%3' champ Num %2 = '%1'").arg(fieldName, QString::number(i), tableName).toAscii();
      outMessage( err);
     }
 for ( i=0; i<nb; ++i)
     {if (i<nb-1) prepare         +=" ?,";
 ...
 err = tr("Nombre de lignes ecrites : %1 sur %2").arg(QString::number(nbWrited), QString::number(nbRows)).toAscii();
 outMessage( err);
 file.close();
 return nbWrited;
}
