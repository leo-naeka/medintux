 ...
 */
QStringList   C_BDM_DatasempPlugin::getPosologiesFromCIP ( C_PatientCtx *pC_PatientCtx, const QString &cip )
                  ...
                  { if (drug_epp_flag==sexe)          // Poso conseillee pour le sexe en cours
                       {  isSexResponse = 0;
                          if (q_min  && q_min<drug_min)         // alert quantite en dessous du minimum conseille pour ce sexe
                             { interResultPairList.append( C_InterResultPair(  s_q_min  , "QUANT", tr("DOSE %1 %2 INCOMPATIBLE POUR LE SEXE %3").arg(s_q_min, drug_qu_unit, s_sexe) ,
                                                                               id       , "CIP" , drugName,
                                                                               tr("Dose prescrite minimum %1 trop faible.<br/>"
                               ...
                               { cim10  = query.value(4).toString().remove('.');
                                 if ( Datasemp_isThisCim10Code_in_Cim10List(cim10, cim10List))
                                    { cimMap[cim10] =  C_InterResultPair(  cim10    , "CIM", CGestIni::Utf8_Query(query, 3)+ " ("+query.value(4).toString()+")",
                                                                           cip      , "CIP", drugName,
                                                                           tr("Etat physiopathologique du patient : <br/>")+CGestIni::Utf8_Query(query, 1)+"<br/>"+CGestIni::Utf8_Query(query, 2),"IT_CIM", Datasemp_LevelToLevelStr(niveau)
                                                                        );
                                    } // if ( Datasemp_isThisCim10Code_in_Cim10List(cim10, cim10List))
                               } // case
                          } // switch(niveau)
...
int C_BDM_DatasempPlugin::Datasemp_isThisInteractionPresentInListe(int cipToExclude, int itCode, const QMap <int, INTER_STRUCT> &cip_interMultiMap)
{   INTER_STRUCT inter;
 ...
 *  \return QList<C_InterResultPair> result interaction pair list
 */
QList<C_InterResultPair> C_BDM_DatasempPlugin::getInteractionRedundancy( const QStringList &cipList)
{   QList<C_InterResultPair>            interResultPairList;
            ...
            if ( nb >= 2 )   // une redondance est trouvee car deux CIP avec la meme substance
               { QString title = tr(" <b>%1</b> redondances de subtance possibles ").arg(QString::number(nb));
                 ...
                 interResultPairList.append( C_InterResultPair( ""       , "REDON", title,
                                                                ""       , "REDON", substName,
                                                                tr("%1 produits avec la même substance <b>%2</b> on \303\251t\303\251 trouv\303\251s :<br/>").arg(QString::number(nb), substName) + avec + "<br/>","REDONDANCES", QString::number(nb)
                                                              )
                                           );
      ...
      while (  query.next() )
            {  int codeTerrain = query.value(0).toInt();
               ...
               if (dopeIsNotEvaluated && ba.length() && ba[0])
                  { title = tr(" Produit pouvant être positif lors des contr\303\264les anti-dopage");
                       ...
                       case 1:  // age
                             {
                                ...
                                if    (level==2988)
                                      ...
                                      { isAlert  =   (d_age >  d_borneInf && d_age < d_borneSup);   // d_borneInf = 0    d_borneSup = 15 les valeurs dans t2G sont exclusives
                                        strAlert = tr("Age du patient <b>%1</b> incompatible car entre <b>%2</b> et <b>%3</b><br/>").arg(ageToDisplay, ageToDisplayInf, ageToDisplaySup);
                                      }
                             ...
                             {  if (borneSup==0) borneSup = 500;   // 500 kg
                                ...
                                if    (level==2988)
                                      ...
                                      { isAlert =   (poids >  borneInf && poids < borneSup);   // borneInf = 0    d_borneSup = 15 les valeurs dans t2G sont exclusives
                                        strAlert = tr("Poids du patient <b>%1 kg</b> incompatible car entre <b>%2</b> et <b>%3</b><br/>").arg(_poids, QString::number(borneInf), QString::number(borneSup));
                                      }
                      ...
                      case 4:  // allaitement (bornes sans signification)
                            { if (allaitement)
                                 { interResultPairList.append( C_InterResultPair( "Z39.1" , "CIM", title,
                                                                                   cip    , "CIP", drugName,
                                                                                   tr("Allaitement : <br/>")+ texte + "<br/>" ,"IT_ALL",      Datasemp_LevelToLevelStr(level)
                                                                                 )
                                                              );
                                 }
                               break;
                            ...
                            { if (borneSup==0) borneSup = 500;   // 500 ml/mn
                              ...
                              if    (level==2988)
                                    ...
                                    { isAlert  =   (clairance >  borneInf && clairance < borneSup);   // borneInf = 0    d_borneSup = 15 les valeurs dans t2G sont exclusives
                                      strAlert = tr("Clairance du patient <b>%1 ml/min</b> incompatible car entre <b>%2</b> et <b>%3</b><br/>").arg(_clairance, QString::number(borneInf), QString::number(borneSup));
                                    }
                              api_level = Datasemp_LevelToLevelInt(level);
                      ...
                      case 6:  // sexe
                            { if (sexe == SexeAll)
                                 {interResultPairList.append( C_InterResultPair(  " "    , "", title,
                                                                                   cip     , "CIP", drugName,
                                                                                   tr("Sexe : <br/>")+title+"<br/>"+ texte + "<br/>" ,"IT_SEX",      Datasemp_LevelToLevelStr(level)
                                                                               )
                                                            );
                                 }
                               break;
...
{   m_position = 0;
    ...
    outSQL_error( *m_pQSqlQuery, "ERREUR  : C_BDM_DatasempPlugin::getDrugList_Start() create index t00.f2 `a2prim`", "", __FILE__, __LINE__);
    //.......... cette requete elimine les produits sans composition ............
 ...
 *  \return QList <C_BDM_IndicationRecord> indications result list
 */
QList <C_BDM_IndicationRecord> C_BDM_DatasempPlugin::Datasemp_getIndications_FromCIP(const QString &cip, int filter )
{
  QList <C_BDM_IndicationRecord> retList;
