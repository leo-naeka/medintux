...
//--------------------------------- set_taille -----------------------------------------------------
void    C_PatientCtx::set_taille(const QString &taille)
{   m_taille       = taille;
...
{   int           nb_jours  = 0;
    ...
    // - Chez l'homme = 1.25 x Poids (kg) x (140-age) / creatinine (µmol/l)
    // - Chez la femme = 1.04 x Poids (kg) x (140-age) / creatinine (µmol/l)
    ...
    if  (pos   != -1)
        {vMax   = v_norm.mid(pos+1).toFloat();
         v      = value.toFloat();
         if (v >= vMax) return  -1;                                         // RETOURNER  -1 de l'insufisance renale (nb de jours non connus)
        }
