    ...
    : QObject(parent,name)
{
	...
	int i = 0;
	if (CGestIni::Param_ReadParam(Global_ParamData, "ConnexionFTP", "Serveur", &adress, &port)==0) {
	    i++;
	}
	if (CGestIni::Param_ReadParam(Global_ParamData, "ConnexionFTP", "Utilisateur", &user, &pwd)==0) {
	    i++;
	}
	if (CGestIni::Param_ReadParam(Global_ParamData, "ConnexionFTP", "Repertoire", &folder)==0) {
	    i++;
	}
	// *** CONNECTION ***
	if (i == 3) {
...
void MyFtp::LetScanFTP()
{
    ...
    emit ftpStateChanged(tr("Transfert en cours"));
...
void MyFtp::stateFTP_changed(int etat)
{
	...
	emit ftpStateChanged(tr("Deconnecté"));
	break;
    case CFtp::HostLookup:
	emit ftpStateChanged(tr("Recherche de l'hote"));
	break;
    case CFtp::Connecting:
	emit ftpStateChanged(tr("Connexion en cours"));
	break;
    case CFtp::Connected:
	// gestion de la couleur de la led
	emit ftpStateChanged(tr("Connecté"));
	break;
    case CFtp::LoggedIn: // l'utilisateur est authentifié
	emit ftpStateChanged(tr("Utilisateur connecté"));
	// connection des signaux et slots du FTP
	...
	break;
    case CFtp::Closing:
	emit ftpStateChanged(tr("Fermeture connexion"));
	break;
    }
    return;
}
...
void MyFtp::Finished(int id,bool error)
{
    ...
    if (command->m_Command == tr("Téléchargement vers Client")) {
    ...
    if (command->m_Command == tr("Effacement d'un fichier")) {
    ...
    if (command->m_Command == tr("Changement de répertoire")) {
	QString format_liste_unix;
	if (CGestIni::Param_ReadParam(Global_ParamData, "ConnexionFTP", "ListFormatUnix", &format_liste_unix)==0) {
    ...
    if (command->m_Command == tr("Commande personnalisée")) {
	LetScanFTP();
    }
...
void MyFtp::Started(int id)
{
	...
	complementCommand  = tr(" de ") + ancienName + tr(" renommé en ") + newName;
	emit ftpStateChanged(tr("Transfert en cours"));

    } else if (command->m_Command == tr("Effacement d'un fichier")) {

	CFtpInfo info = retrieveFileToRemoveWithIndex(id);
	ancienName = info.name();
	complementCommand  = tr(" nommé ") + ancienName;
	emit ftpStateChanged(tr("Transfert en cours"));

    }
    toLog(tr("Début ") + QString::number(id) + " : " + command->m_dateDebut + " : " + command->m_Command + complementCommand);
...
QString MyFtp::commandToString(CFtp::Command command)
{
	...
	valeur = tr("Ouverture Connection");
	break;
    case CFtp::Login:
	valeur = tr("Authentification");
	break;
    case CFtp::Close:
	valeur = tr("Fermeture Connection");
	break;
    case CFtp::List:
	valeur = tr("Listage");
	break;
    case CFtp::Cd:
	valeur = tr("Changement de répertoire");
	break;
    case CFtp::Get:
	valeur = tr("Téléchargement vers Client");
	break;
    case CFtp::Put:
	valeur = tr("Téléchargement vers Serveur");
	break;
    case CFtp::Remove:
	valeur = tr("Effacement d'un fichier");
	break;
    case CFtp::Mkdir:
	valeur = tr("Création d'un répertoire");
	break;
    case CFtp::Rmdir:
	valeur = tr("Effacement d'un répertoire");
	break;
    case CFtp::Rename:
	valeur = tr("Renommage d'un fichier");
	break;
    case CFtp::RawCommand:
	valeur = tr("Commande personnalisée");
	break;
    }
    return valeur;
}
...
QString MyFtp::errorToString(CFtp::Error error)
{
    ...
    switch(error) {
    case CFtp::NoError:
	valeur = tr("Aucune erreur");
	break;
    case CFtp::HostNotFound:
	valeur = tr("Serveur non trouvé");
	break;
    case CFtp::ConnectionRefused:
	valeur = tr("Connection refusée");
	break;
    case CFtp::NotConnected:
	valeur = tr("Non connectée");
	break;
    case CFtp::UnknownError:
	valeur = tr("Erreur inconnue");
	break;
    }
    return valeur;
}
...
void MyFtp::ListChanged(const QUrlInfo &objet)
{
    ...
    // si c'est un fichier
    if (objet.isFile()) {
	toLog(tr("Fichier    : ") + objet.name() + espace.fill(' ',6 - taille.length()) + taille + tr(" Octets, du ") + date);
...
void MyFtp::checkDownload()
{
    ...
    toLog(tr("checkDownload   ") + element.name() + " : " + QString::number(element.size()) + tr(" Octets"));
...
void MyFtp::DownloadFile(CFtpInfo info)
{
    ...
    CFtpFileInfoList::Iterator it = m_FileToDownload.find(info);
    ...
    QString NewName = GlobalPathAppli +  TEMP_FOLDER + QDir::separator() + tr("labo-") + time.toString("yyyyMMddhhmmsszzz") + ".hpr";
...
void MyFtp::uselessFile( QString newName )
{
    ...
    toLog(tr("Erreur ") + QString::number(info.serial()) + tr(" : Fichier de taille 0 : ") + info.name() + tr(" -> ") + info.newName());
