...
{ //................................. methode QSqlCursor .....................................
  ...
  //synchroWriteMofidication(m_DOSS_NOTE_TBL_NAME, primKey, m_DOSS_NOTE_PK, "D");
  CloseBase();
  return ok;
}
...
{ QString pk  = "";
  ...
  //if ( (ok = query.exec()))
     //{ //requete  = "SELECT LAST_INSERT_ID() FROM ";
      ...
      //QSqlQuery queryPK(requete , m_DataBase );
      //if (queryPK.isActive() && queryPK.next())  pk = queryPK.value(0).toString();
      //if (pk.length()) synchroWriteMofidication(m_DOSS_NOTE_TBL_NAME, pk, m_DOSS_NOTE_PK, "C");
    //}
  CloseBase();
  return ok;
/*
...
{QString ref_pk;
 ...
 // if (mode==CMoteurBase::nomade) synchroWriteMofidication(m_DOSS_INDEX_TBL_NAME, refDossPk, m_DOSS_INDEX_PRIM_KEY, "C");
  //........................ recuperer la clef ...........................................
  return ok;
}
...
{//....................... addapter le comportement à un éventuel nomadisme ...............................................
 ...
 // devrait jamais à avoir à marquer ça ici
 //if (mode==CMoteurBase::nomade) synchroWriteMofidication(m_DOSS_IDENT_TBL_NAME, id_doss, "D");
...
{ //............... ouvrir la base  de destination..................................................
 QSqlDatabase *destMasterDB = QSqlDatabase::addDatabase( driver , baseLabel);
 if (destMasterDB<=0) return 0;
 ...
 //             a priori tous les pk doivent être les memes
 //             pour l'update et le create

 //.............. on commence par les effacements des dossiers à effacer ........................................................
 ...
 //................. effacer les tables ................................................................................................
 synchroTablesDelete(destMasterDB);
 //.............. on poursuit par les mises à jour des tables à mettre à jour ........................................................
 synchroTablesUpdate(destMasterDB);
 //.............. on poursuit par les creations des tables à creer ........................................................
 synchroTablesCreate(destMasterDB);
 CloseBase();
 destMasterDB->close();
 QSqlDatabase::removeDatabase ( destMasterDB );
 return 1;
}

//-------------------------------------- synchroTablesCreate ----------------------------------------------------------------------------
void  CMoteurBase::synchroTablesCreate(QSqlDatabase *destMasterDB)
{QStringList tablesList = synchroGetTablesList();
 //............... rechercher pour chaque table ..............................
 //               un eventuel create à faire
 for (int i = 0; i < (int)tablesList.size(); ++i)
     {synchroTablesCreateDispatcher(tablesList[i],destMasterDB);
     }
}

//-------------------------------------- synchroTablesCreateDispatcher ----------------------------------------------------------------------------
void  CMoteurBase::synchroTablesCreateDispatcher(const QString &table, QSqlDatabase *destMasterDB)
{     QSqlRecord rec  = destMasterDB->record (table );
      ...
      if  (query.isActive())
          {while (query.next())
                 {if (table ==m_DOSS_INDEX_TBL_NAME)
                     {synchroTableIndexFichePatientCreate(table, rec, query.value(0).toString(), query.value(1).toString(), destMasterDB);
                     }
                  else if (table == m_DOSS_NOTE_TBL_NAME)
                     {synchroTableNoteCreate(table, rec, query.value(0).toString(), query.value(1).toString(), destMasterDB);
                     }
                  else if (table == m_DOSS_RUB_HEAD_TBL_NAME)
                     {synchroTableRubriqueHeadBlobsCreate(table, rec, query.value(0).toString(), query.value(1).toString(), destMasterDB);
                     }
                  else if (table != m_DOSS_RUB_DATA_TBL_NAME &&  // creation traitée avec les cas particuliers plus haut : synchroTableRubriqueHeadBlobsCreate
                           table != m_DOSS_IDENT_TBL_NAME )      // creation traitée avec les cas particuliers plus haut : synchroTableIndexFichePatientCreate
                     {synchroTableCreate(table, rec, query.value(0).toString(), query.value(1).toString(), destMasterDB);
                     }
                 }
          }
      else
...
void  CMoteurBase::synchroTableNoteCreate(const QString &table, QSqlRecord &rec, const QString &primKey, const QString &primKeyName,  QSqlDatabase *destMasterDB)
{ QString dossDestPk      = "";
  QString requete         = "";
  QString guid            = "";
  ...
  QSqlQuery querySrc(requete , m_DataBase );
...
// cas particulier des rubriques ou les references pk du blobs lie et de l'index nom prenom lie doit etre inscrit
//              les pk  deja en place ne seront pas ceux crees sur le master.
void  CMoteurBase::synchroTableRubriqueHeadBlobsCreate(const QString &table, QSqlRecord &rec, const QString &primKey, const QString &primKeyName,  QSqlDatabase *destMasterDB)
{ QString newPk           = "";
  QString newPkDoss       = "";
  ...
  //                   et recuperer sa pk
  newPk = synchroTableCreate(m_DOSS_RUB_DATA_TBL_NAME, destMasterDB->record (m_DOSS_RUB_DATA_TBL_NAME ), primKeyBlobsSrc, m_DOSS_RUB_DATA_PRIMKEY,  destMasterDB);
...
void  CMoteurBase::synchroTableIndexFichePatientCreate(const QString &tableInd, const QSqlRecord &recInd, const QString &primKeyInd, const QString &primKeyNameInd,  QSqlDatabase *destMasterDB)
{ QString newPk   = "";
  QString requete = "";
  //................... recuperer le guid du patient .........................................................................
  //                    car c'est lui qui va nous guider pour recuperer
  ...
  QString truc = synchroTableCreate(tableInd, recInd, primKeyInd, primKeyNameInd,  destMasterDB);
...
/*
//-------------------------------------- synchroTableCreate ----------------------------------------------------------------------------
QString  CMoteurBase::synchroTableCreate(const QString &table, const QSqlRecord &rec, const QString &primKey, const QString &primKeyName,  QSqlDatabase *destMasterDB)
{ int nb          = rec.count ();
  int i           = 0;
  QString requete = "";
  ...
  QSqlQuery queryDst(destMasterDB);
  queryDst.prepare(prepare);
  if (querySrc.isActive() && querySrc.next())
     {for (i = 0; i<nb; ++i)
...
//-------------------------------------- synchroTablesDelete ----------------------------------------------------------------------------
void  CMoteurBase::synchroTablesDelete(QSqlDatabase *destMasterDB)
{QStringList tablesList = synchroGetTablesList();
 QString          table = "";
 //............... rechercher pour chaque table ..............................
 //               un eventuel update à faire
 for (int i = 0; i < (int)tablesList.size(); ++i)
     {table           = tablesList[i];
      QSqlRecord rec  = destMasterDB->record (table );
      ...
      if  (query.isActive())
          {while (query.next())
                 {synchroTableDelete(table, rec, query.value(0).toString(), query.value(1).toString(), destMasterDB);
                 }
          }
      else
...
//-------------------------------------- synchroTableDelete ----------------------------------------------------------------------------
void  CMoteurBase::synchroTableDelete(const QString &table, const QSqlRecord &, const QString &primKey, const QString &primKeyName,  QSqlDatabase *destMasterDB)
{ //int nb = rec.count ();
  //int i  = 0;
...
//-------------------------------------- synchroTablesUpdate ----------------------------------------------------------------------------
void  CMoteurBase::synchroTablesUpdate(QSqlDatabase *destMasterDB)
{QStringList tablesList = synchroGetTablesList();
 QString          table = "";
 //............... rechercher pour chaque table ..............................
 //               un eventuel update à faire
 for (int i = 0; i < (int)tablesList.size(); ++i)
     {table           = tablesList[i];
      ...
      if  (query.isActive())
          {while (query.next())
                 {synchroTableUpdate(table, rec, query.value(0).toString(), query.value(1).toString(), destMasterDB);
                 }
          }
      else
...
//-------------------------------------- synchroTableUpdate ----------------------------------------------------------------------------
void  CMoteurBase::synchroTableUpdate(const QString &table, const QSqlRecord &rec, const QString &primKey, const QString &primKeyName,  QSqlDatabase *destMasterDB)
{ int nb = rec.count ();
  int i  = 0;
  //.......... verifier que la table de destination existe encore ......................
  //           a pu etre effacee pendant l'absence du nomade
  ...
  if ( !query.next())     {synchroWriteMofidication(table, primKey, primKeyName, "C");            // si pas trouve  on le met en creation (a soumettre à une option ?)
                           //synchroTablesCreateDispatcher(table, destMasterDB);
                           qDebug(QString("synchroTableUpdate()::enregistrement de destination non trouvé --> creation de l'enregistrement\n")+           requete);
                           return;
                          }
...
{QString nom, prenom, guid;
 ...
 synchroWriteMofidication(m_DOSS_IDENT_TBL_NAME, primKeyPat, m_DOSS_IDENT_REF_PK,   "D"); // la pk de l'identite est la meme que celle de l'index (placé de force à la création)
...
{ //............... ouvrir la base  ..................................................
 if  (OpenBase()==0)  return ;
 QString requete ( "DROP TABLE IF EXISTS ") ;
 QSqlQuery query_Drop(requete + tableName+"_SYNC",   m_DataBase);
 CloseBase();
}
...
{tableName      += "_SYNC";
 ...
 if  (query.isActive())
     ...
     {OutSQL_error(query, "synchroWriteMofidication select( )" , requete / *=0* /);
     }
 return status;
}
*/
...
{ //............... ouvrir la base  ..................................................
 ...
 if (pk.length())
    {if (status == "C" && modif=="U")  return;   // si en creation garder l'attribut : doit etre cree
     ...
     QSqlQuery queryU(requete , m_DataBase );
    ...
    {//requete =  QString("INSERT INTO ") + tableName   + "_SYNC (Pk_to_modified , Modification, TableName) VALUES ('" + pk_toWrite + "','" + modif + "','"+tableName+"' )";
     ...
     QSqlQuery query_I(requete, m_DataBase);
...
{tableName      += "_SYNC";
 ...
 QString requete  = " DELETE FROM ";
         requete +=  tableName + " WHERE Pk_to_modified ='" + pk_toDelete + "'";
 QSqlQuery query(requete , m_DataBase );
...
{synchroDropTable(tableName);
 ...
 requete +=        ");";
 QSqlQuery query_sync(requete, m_DataBase);
