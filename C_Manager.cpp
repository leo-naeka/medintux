...
{   bool finFSE = false;
    ...
    if (finFSE)
        ...
        m_timerFSE->stop();                                     // arrÃƒÂªt du timer d'attente du fichier Facture.par
        QTreeWidget *pQlistView = m_pGUI->listView_Vitale;
        pQlistView->clear();
...
void C_Manager::Slot_pushButton_FSE()
{
   ...
   if (moduleName=="PYXVITAL")    // test inutile
      {
       // vÃ©rification qu'une FSE n'est pas dÃ©jÃƒ  en cours de saisie
       if (m_FSEenCours)
          {
           ...
           if (ret == 1) return;
           // voir comment effacer le formulaire Pyxvital si acte dÃ©jÃƒ  saisi...
      ...
      if (!patientOK)
          ...
          */
          ...
          // appel Ãƒ  AixVitale (qui lance Pyxvital) pour saisie des actes et gÃ©nÃ©ration de la FSE.
          //--- dÃ©but CZ_Cpta
